...
class Arrays(Abstract):
    ...
    def interpretar(self, arbol, tabla):
        if tabla.getTabla(self.expresion) != None:
            ...
            return resultado
        simbolo = Simbolo("array", self.expresion, self.fila, self.columna)
...
class AccessArray(Abstract):
    ...
    def interpretate(self, tree, table):
        var = None
        result = None
        indexB = self.indexB.interpretate(tree, table)
        indexE = None

        if self.indexE is not None:
            indexE = self.indexE.interpretate(tree, table)

        if indexB.type == "number":
            if indexB.value > 0:
                if isinstance(self.id, AccessArray):
                    var = self.id.interpretate(tree, table)
                else:
