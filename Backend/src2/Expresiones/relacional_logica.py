...
class Relacional_Logica(Abstract):
    ...
    def interpretar(self, arbol, tabla):
        ...
        if (self.op != '&&' and self.op != '||' and self.op != '!'):
            generador.addComment("Compilacion de Expresion Relacional1")

            left = self.op_izq.interpretar(arbol, tabla)
            print(self.op_izq)
            if isinstance(left, Error): 
                print("Semantico", "No se puede utilizar la expresion booleana en iz: ", left, self.fila, self.columna)
                print(left.desc, left.columna)
                return left
            right = None
            ...
            generador.addComment("Fin de compilacion de Expresion Relacional1")
            generador.addSpace()
            ...
            generador.addComment("Compilacion de Expresion Relacional2")
            self.checkLabels()
            print("izquierda ", self.op_izq ," derecha: ", self.op_der, self.op)
            lblAndOr = ''
            if self.op == '&&':
                ...
                lblNot = self.op_izq.interpretar( arbol, tabla)
                if isinstance(lblNot, Error):
                    print("Semantico", "No se puede utilizar la expresion booleana en lnot: ", self.fila, self.columna)

                    return lblNot

                if lblNot.getTipo() != 'boolean':
                    print("Semantico", "No se puede utilizar la expresion booleana en: ", self.fila, self.columna)

                    return Error("Semantico", "No se puede utilizar la expresion booleana en: ", self.fila, self.columna)
