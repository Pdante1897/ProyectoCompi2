from src.Instrucciones.AccesoA import AccessArray
from src.Instrucciones.Break import Break
from src.Instrucciones.Continue import Continue
from src.Instrucciones.AccesoS import AccesoS
from src.Instrucciones.DeclaraS import DeclareStruct, CreaStruct, DeclareStruct2
from src.Instrucciones.AsignaStruct import AsignaStruct
from src.Instrucciones.Return import Return
from src.Instrucciones.Nativas import Nativa
...
from src.Instrucciones.Imprimir import Imprimir, Imprimir2
...
from src.Instrucciones.struct import Struct
from src.Instrucciones.arrays import Arrays
from src.Instrucciones.condicional_if import If
from src.Instrucciones.ciclo_for import For
from src.Instrucciones.ciclo_forA import ForArray
from src.Instrucciones.ciclo_while import While
from src.Expresiones.relacional_logica import Relacional_Logica
from src.Instrucciones.Declarar_variables import Declaracion_Variables, Declaracion_Variables2, Declaracion_Variables3, Declaracion_Variables4
from src.Instrucciones.Asignaciones import Asignacion_Variables, Asignacion_incrementable
from src.Instrucciones.Funciones import Metodo, Llamada
from src.Estructuras.tablasimbolos import TablaSimbolos
from src.Estructuras.arbol import Arbol
from src.Instrucciones.Typeof import TypeOf
...
def p_declaracion_4(t):
    ...
    print("aqui SE ESTA QUEDANDO DECLARACION")
...
def p_parametrof(t):
    ...
    if len(t) == 2:
        ...
        t[0] = {'tipo': t[3], 'id': t[1], 'array': True}
    else:
        t[0] = {'tipo': t[4], 'id': t[2]}
...
def p_ciclo_for5(t):
    ...
    t[0] = ForArray(t[4], t[6], t[9], t.lineno(1), get_Columna(input, t.slice[1]))
...
def p_arrays_Declaracion(t):
    '''arrays_asigna : LET ID CORIZQ CORDER
                 ...
                 | IGUAL expresion'''
    if len(t) == 6:
        t[0]= Arrays(t[2],t[5],t.lineno(2), get_Columna(input, t.slice[2]))
    elif len(t) == 5:
        ...
        if t[0] == '[':
            ...
            t[0]= Arrays(t[2],None,t.lineno(2), get_Columna(input, t.slice[2]))
    else:
       # t[0] = (t[1], t[4],t.lineno(1), get_Columna(input, t.slice[1]))        
 
        t[0] = Arrays(None,t[2],t.lineno(1), get_Columna(input, t.slice[1]))        
...
def p_expresion_array(t):
    ...
    if len(t) == 2:
        ...
        t[0]=AccessArray(t[1],t[3], None , t.lineno(1), get_Columna(input, t.slice[1]))

'''def p_identificador(t):
    'expresion : ID'
    print("aqui no tiene que ser    "+t[1])
    t[0] = Identificador(t[1], t.lineno(1), get_Columna(input, t.slice[1]), None)'''
...
def p_expresion_array2(t):
    ...
    t[0] = DeclareArray(t[2],  t.lineno(1), get_Columna(input, t.slice[1]))
    #t[0]= t[2]
    

def p_break(t):
    'prod_break : BREAK'
    t[0] = Break(t.lineno(1), get_Columna(input, t.slice[1]))

def p_continue(t):
    'prod_continue : CONTINUE'
    t[0] = Continue(t.lineno(1), get_Columna(input, t.slice[1]))
...
def correr(entrada):
    ...
    tsg = TablaSimbolos()
    ast.setTsglobal(tsg)
    #graph = generar_codigo_graphviz(instrucciones)  
    #print(graph)
    if True:
